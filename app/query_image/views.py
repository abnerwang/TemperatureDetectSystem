...
@query_image.route('/query_co_image_info', methods=['GET', 'POST'])
def query_co_image_info():
    ...
    if defect_type == '所有类型':
        ...
        co_image2 = co_image1.query.filter_by(defect_type=defect_type)

    if device_type == '所有类型':
        co_image3 = co_image2
    else:
        co_image3 = co_image2.query.filter_by(device_type=device_type)

    if power_company_province == '':
        co_image4 = co_image3
    else:
        co_image4 = co_image3.query.filter_by(power_company_province=power_company_province)

    if power_company_cityorcounty == '':
        co_image5 = co_image4
    else:
        co_image5 = co_image4.query.filter_by(power_company_cityorcounty=power_company_cityorcounty)

    if suborlineorzone_name == '':
        co_image6 = co_image5
    else:
        co_image6 = co_image5.query.filter_by(suborlineorzone_name=suborlineorzone_name)

    images = co_image6.query.filter(co_image6.detection_date.between(start_date, end_date)).all()
